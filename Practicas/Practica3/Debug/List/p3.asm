...
; 0000 0031 DDRC=(0<<DDC7) | (0<<DDC6) | (0<<DDC5) | (0<<DDC4) | (0<<DDC3) | (0<<DDC2) | (0<<DDC1) | (0<<DDC0);
	OUT  0x14,R30
; 0000 0032 // State: Bit7=P Bit6=P Bit5=P Bit4=P Bit3=P Bit2=P Bit1=P Bit0=P
; 0000 0033 PORTC=(1<<PORTC7) | (1<<PORTC6) | (1<<PORTC5) | (1<<PORTC4) | (1<<PORTC3) | (1<<PORTC2) | (1<<PORTC1) | (1<<PORTC0);
	LDI  R30,LOW(255)
	OUT  0x15,R30
; 0000 0034 
...
; 0000 0039 PORTD=(1<<PORTD7) | (1<<PORTD6) | (1<<PORTD5) | (1<<PORTD4) | (1<<PORTD3) | (1<<PORTD2) | (1<<PORTD1) | (1<<PORTD0);
	LDI  R30,LOW(255)
	OUT  0x12,R30
; 0000 003A 
...
; 0000 0088 while (1){
_0x3:
; 0000 0089 //Display de 7 segmentos cátodo común
; 0000 008A if(!PIND.4){  //Si el pin 4 del puerto D está en 0 -> Convertidor BCD
	SBIC 0x10,4
	RJMP _0x6
; 0000 008B catodo = PIND & 0x0f; //Enmascaramos los 4 bits menos significativos del puerto A ya que los demás no interesan
	IN   R30,0x10
	ANDI R30,LOW(0xF)
	MOV  R4,R30
; 0000 008C if(catodo < 10){
	LDI  R30,LOW(10)
	CP   R4,R30
	BRSH _0x7
; 0000 008D PORTB = tablaBCD[catodo];
	MOV  R30,R4
	LDI  R31,0
	SUBI R30,LOW(-_tablaBCD*2)
	SBCI R31,HIGH(-_tablaBCD*2)
	LPM  R0,Z
	OUT  0x18,R0
; 0000 008E }
; 0000 008F if(catodo >= 10){ //Si lo que leemos es mayor o igual de 10 que dibuje en el display una E de ERROR
_0x7:
	LDI  R30,LOW(10)
	CP   R4,R30
	BRLO _0x8
; 0000 0090 PORTB = tablaBCD[10];
	__POINTW1FN _tablaBCD,10
	LPM  R0,Z
	OUT  0x18,R0
; 0000 0091 }
; 0000 0092 }
_0x8:
; 0000 0093 if(PIND.4){   //Si el pin 4 del puerto D está en 1 -> Convertidor HEX
_0x6:
	SBIS 0x10,4
	RJMP _0x9
; 0000 0094 catodo = PIND & 0x0f;
	IN   R30,0x10
	ANDI R30,LOW(0xF)
	MOV  R4,R30
; 0000 0095 PORTB = tablaHEX[catodo];
	LDI  R31,0
	SUBI R30,LOW(-_tablaHEX*2)
	SBCI R31,HIGH(-_tablaHEX*2)
	LPM  R0,Z
	OUT  0x18,R0
; 0000 0096 }
; 0000 0097 
; 0000 0098 //Display de 7 segmentos ánodo común
; 0000 0099 if(!PINC.4){
_0x9:
	SBIC 0x13,4
	RJMP _0xA
; 0000 009A anodo = PINC & 0x0f;
	IN   R30,0x13
	ANDI R30,LOW(0xF)
	MOV  R5,R30
; 0000 009B if(anodo < 10){
	LDI  R30,LOW(10)
	CP   R5,R30
	BRSH _0xB
; 0000 009C PORTA = ~tablaBCD[anodo];
	MOV  R30,R5
	LDI  R31,0
	SUBI R30,LOW(-_tablaBCD*2)
	SBCI R31,HIGH(-_tablaBCD*2)
	RCALL SUBOPT_0x0
; 0000 009D }
; 0000 009E if(anodo >= 10){
_0xB:
	LDI  R30,LOW(10)
	CP   R5,R30
	BRLO _0xC
; 0000 009F PORTA = ~tablaBCD[10];
	__POINTW1FN _tablaBCD,10
	RCALL SUBOPT_0x0
; 0000 00A0 }
; 0000 00A1 }
...
_0xA:
	SBIS 0x13,4
	RJMP _0xD
; 0000 00A3 anodo = PINC & 0x0f;
	IN   R30,0x13
	ANDI R30,LOW(0xF)
	MOV  R5,R30
; 0000 00A4 PORTA = ~tablaHEX[anodo];
	LDI  R31,0
	SUBI R30,LOW(-_tablaHEX*2)
	SBCI R31,HIGH(-_tablaHEX*2)
	RCALL SUBOPT_0x0
